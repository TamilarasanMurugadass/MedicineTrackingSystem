...
public class InventoryService : IInventoryService
{
    ...
    public async Task<IEnumerable<InventoryTransactionDto>> GetAllTransactionsAsync()
    {
        ...
        return _mapper.Map<IEnumerable<InventoryTransactionDto>>(transactions.OrderByDescending(t => t.CreatedAt));
    }

    public async Task<InventoryTransactionDto?> GetTransactionByIdAsync(int id)
    {
        var transaction = await _unitOfWork.InventoryTransactions.GetByIdAsync(id);
    ...
    {
        var transactions = await _unitOfWork.InventoryTransactions.FindAsync(t => t.MedicineBatchId == batchId);
        return _mapper.Map<IEnumerable<InventoryTransactionDto>>(transactions.OrderByDescending(t => t.CreatedAt));
    }

    public async Task<IEnumerable<InventoryTransactionDto>> GetTransactionsByUserAsync(string userId)
    {
        var transactions = await _unitOfWork.InventoryTransactions.FindAsync(t => t.CreatedBy == userId);
        return _mapper.Map<IEnumerable<InventoryTransactionDto>>(transactions.OrderByDescending(t => t.CreatedAt));
    }

    public async Task<IEnumerable<InventoryTransactionDto>> GetTransactionsByTypeAsync(TransactionType type)
    {
        var transactions = await _unitOfWork.InventoryTransactions.FindAsync(t => t.TransactionType == type);
        return _mapper.Map<IEnumerable<InventoryTransactionDto>>(transactions.OrderByDescending(t => t.CreatedAt));
    }
    ...
    public async Task<IEnumerable<UsageSummaryDto>> GetUsageSummaryAsync(DateTime? startDate = null, DateTime? endDate = null)
    {
        var transactions = await _unitOfWork.InventoryTransactions.FindAsync(t =>
            ...
            (!startDate.HasValue || t.CreatedAt >= startDate) &&
    ...
    public async Task<IEnumerable<UsageSummaryDto>> GetUsageSummaryByMedicineAsync(int medicineId, DateTime? startDate = null, DateTime? endDate = null)
    {
        var transactions = await _unitOfWork.InventoryTransactions.FindAsync(t =>
            t.TransactionType == TransactionType.OUT &&
            t.MedicineBatch.MedicineId == medicineId &&
            (!startDate.HasValue || t.CreatedAt >= startDate) &&
            (!endDate.HasValue || t.CreatedAt <= endDate));
